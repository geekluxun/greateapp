DELETE FROM t_cash_order
WHERE apply_id IN (SELECT tca.apply_id
                   FROM t_cash_apply tca
                   WHERE tca.mobile_no = '13817806136' AND tca.prod_code = 'QYL');
...
INSERT INTO `t_gate_payments` (
    ...
    SELECT
        ...
        `create_time`,
        `update_time`
    FROM t_gate_pay_txn a
    WHERE a.pay_time <= '20161208235959' AND a.trans_type = '52';
...
UPDATE t_qyl_order q INNER JOIN
    (SELECT
         loan_order_id AS loan_order_ref,
         '10'          AS STATUS,
         update_time,
         b.versions
     FROM lo_loan_order b
     WHERE b.business_code = 'QYL'
           AND b.loan_status = '01'
           AND EXISTS(SELECT 1
                      FROM lo_repay_info a
                      WHERE a.`status` = '11' AND a.business_code = 'QYL' AND a.loan_order_ref = b.loan_order_id))
...
INSERT INTO t_user_contacts_qyl SELECT *
